...
with program() as pulsed_odmr:
    ...
    with stream_processing():
        ...
        # Calculate difference and average it
        (counts_st.buffer(len(f_vec)) - counts_ref_st.buffer(len(f_vec))).average().save("counts_diff")
        n_st.save("iteration")
...
if simulate:
    ...
    results = fetching_tool(job, data_list=["counts", "counts_ref", "counts_diff", "iteration"], mode="live")
    # Live plotting
    fig = plt.figure()
    ...
    while results.is_processing():
        # Fetch results
        counts, counts_ref, counts_diff, iteration = results.fetch_all()
        # Progress bar
        progress_counter(iteration, n_avg, start_time=results.get_start_time())
        # Plot data
        plt.cla()
        plt.plot((NV_LO_freq * 1 + f_vec) / u.MHz, counts_diff / 1000 / (meas_len_1 * 1e-9), label="signal - reference")
        plt.xlabel("MW frequency [MHz]")
        plt.ylabel("Intensity [kcps]")
        plt.title("pulsed ODMR (averaged difference)")
        plt.legend()
        plt.pause(0.1)
    #turn off SRS out
    sg384.ntype_on(0)
    ...
    save_data_dict.update({"counts_diff_data": counts_diff})
